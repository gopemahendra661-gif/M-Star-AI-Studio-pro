...
import { exportRoastImage } from '../utils/imageExporter';
...
const RoastImageCard: React.FC<RoastImageCardProps> = ({ content, onClose }) => {
  ...
  const handleAction = async (action: 'download' | 'share') => {
    ...
    // Small delay to allow UI to settle if needed
    setTimeout(async () => {
      await exportRoastImage('roast-card-capture', action);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fadeIn">
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50 dark:bg-slate-950 flex items-center justify-center">
